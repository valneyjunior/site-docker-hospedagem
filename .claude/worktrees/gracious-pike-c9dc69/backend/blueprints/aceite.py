...
def _salvar_aceite(protocolo, nome, email, cpf_cnpj, empresa, plano, plan_id, domain,
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO aceites
                        (protocolo, nome, email, cpf_cnpj, empresa, plano, plan_id, domain,
...
def confirmar_aceite():
    ...
    # Domínio obrigatório para planos de hospedagem de sites e dedicados
    _planos_com_dominio = {"sites_basico_mensal", "sites_plus_mensal", "sites_pro_mensal",
                           "ded_essencial_mensal", "ded_profissional_mensal", "ded_enterprise_mensal"}
    if plan_id in _planos_com_dominio and not domain:
        return jsonify({"erro": "Informe o domínio para o plano de hospedagem."}), 400
    ...
    _salvar_aceite(
        meta["protocolo"], nome, email, cpf_cnpj, empresa, plano_nome, plan_id, domain,
